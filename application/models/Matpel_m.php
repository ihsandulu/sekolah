...
class matpel_M extends CI_Model
{
	...
	public function data()
	{
		...
		if ($this->input->post("create") == "OK") {
			...
			foreach ($kelasid as $e => $f) {
				$inputa[$e] = $f;
			}
			$inputa["matpel_id"] = $insertid;
			$this->db->insert("matpelkelas", $inputa);
		...
		if ($this->input->post("change") == "OK") {
			foreach ($this->input->post() as $e => $f) {
				if ($e != 'change' && $e != 'matpel_picture' && $e != 'kelas_id') {
					$input[$e] = $this->input->post($e);
				}
			}
			$this->db->update("matpel", $input, array("matpel_id" => $this->input->post("matpel_id")));
			
			$this->db->delete("matpelkelas", array("matpel_id" => $this->input->post("matpel_id")));
			$kelasid = $this->input->post("kelas_id");
			foreach ($kelasid as $e => $f) {
				$inputa["kelas_id"] = $f;
				$inputa["sekolah_id"] = $this->session->userdata("sekolah_id");
				$inputa["matpel_id"] = $this->input->post("matpel_id");
				$this->db->insert("matpelkelas", $inputa);
			}
