...
function kirimpesan(message,number,server,email,password){	
	let pesankirim='';
	setTimeout(() => {
		$.get("https://qithy.my.id/api/token",{email:email,password:password})
		.done(function(data){
			let token = data.token;
			// alert("https://qithy.my.id:8000/send-message?email="+email+"&token="+token+"&message="+message+"&number="+number+"&id="+server);
			$.get("https://qithy.my.id:8000/send-message",{email:email,'token': token, message:message, number:number,id:server})
			.done(function(data){ 
...
function cektelpon(){
	...
	.done(function(data){
		...
		$.each( data, function( key, value ) {
			...
			if(notel<150){
				setTimeout(function () {
					dikirimpesan(value.message,value.number,value.server,value.email,value.password,value.nominal,value.user_id);
					notel=1;
				}, <?=rand(10000,15000);?>);
				notel++;
			}else{
